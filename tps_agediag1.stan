...
parameters{
  ...
  real<lower=0> sigma; // 1/lambda ... i.e. sd not precision (stan likes that more) 
  vector<lower=0>[4] vardelta;
  vector[nyr] delta_raw1; 
  vector[nyr] delta_raw2; 
  vector[nyr] delta_raw3; 
  vector[nyr] delta_raw4; 
  vector[4*nagroup] alpha; // age group specific (ag) and state (k) specific intercept for dx probs --- rule to map to vector [(ag-1)*4 + k - 1]
...
transformed parameters{
  ...
  vector[nyr] delta3; 
  vector[nyr] delta4; 
  real<lower=0> lambda; // smoothing parameters
